...
LABEL maintainer="Kuma Navigation DOCKER MAINTAINER <pony@ponycool.com>"

ENV TIMEZONE=Asia/Shanghai
ENV VHOST_PATH=/etc/nginx/vhost
ENV PHP_FPM_CONF_PATH=/usr/local/etc/php-fpm.d
ENV APP_PATH=/opt/navigation
...
COPY deploy/conf/nginx.vh.navigation.conf "$VHOST_PATH/navigation.conf"
...
RUN set -eux; \
    cd /var/www/html; \
    { \
        echo  '<div>Hello Kuma Navigation</div>'; \
    } | tee index.html; \
    { \
        echo '<?php'; \
        echo '    phpinfo();'; \
